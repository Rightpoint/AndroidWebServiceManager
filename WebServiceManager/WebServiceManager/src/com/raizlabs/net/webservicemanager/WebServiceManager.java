...
import com.raizlabs.baseutils.SimpleEventListener;
...
import com.raizlabs.net.requests.UrlConnectionRequestable;
...
public class WebServiceManager {
	...
	public <RequestType extends HttpUriRequestable, ResultType> ResultInfo<ResultType> doRequest(RequestType request, 
			HttpRequestTranslator<RequestType, ResultType> translator ) {
		beginConnection();
		HttpResponse response = requestQueue.doRequest(request.getHttpUriRequest());
		ResultType result = translator.translateHTTPResponse(request, response);
		endConnection();
		return new ResultInfo<ResultType>(result, new Date(), response);
	}

	public <RequestType extends UrlConnectionRequestable, ResultType> ResultInfo<ResultType> doRequestViaConnection(
			RequestType request, UrlConnectionTranslator<RequestType, ResultType> translator) {
		HttpURLConnection connection = null;
		try {
			beginConnection();
			connection = request.getUrlConnection();
			connection.connect();
			ResultType result = translator.translateConnection(request, connection);
			return new ResultInfo<ResultType>(result, new Date(), connection);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
